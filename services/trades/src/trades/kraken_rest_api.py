...
class KrakenRestAPI:
    ...
    def __init__(self, product_id: str, last_n_days: int):
        ...
        # Get the timestamp in nanoseconds for the last n days
        self.since_timestamp_ns = int(time.time_ns() - last_n_days * 24 * 60 * 60 * 1000000000)
    ...
    def get_trades(self) -> list[Trade]:
        ...
        """
        # Set the right headers and parameters for the request
        headers = {"Accept": "application/json"}
        params: dict[str, str] = {
            "pair": self.product_id,
            "since": str(self.since_timestamp_ns),
        }

        # Get the trades data from the Kraken API
        try:
        ...
        try:
            # Get the trades data
            trade_data = data["result"][self.product_id]
