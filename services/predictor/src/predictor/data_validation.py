...
def validate_data(
    ...
    perc_row_with_missing_data = (len(ts_data) - len(ts_data_without_nans)) / len(ts_data)
    if perc_row_with_missing_data > max_percentage_rows_with_missing_values:
        raise Exception("ts_data has too many rows with missing data. Aborting the training script")
...
#     # '''
#     # Run the Data Drift evaluation preset that will test for shift in column distributions.
#     # Take the first 60 rows of the dataframe as "current" data and the following as reference.
#     # Get the output in Jupyter notebook:
